<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="default.aspx.cs" Inherits="Web.admin.web.pro._default" %>
...
<body id="iframe-body">
    <div class="main">
        ...
        <div style="margin-bottom: 15px;">
            <form name="from" method="get" action="default.aspx">
                <%if (tlist.Count > 0)
                  { %>
                <select name="tid">
                ...
                <%} %>
                <select name="a3">
                    <option value="1" <%=a3==""?"selected='selected'":"" %>>请选择行业</option>
                    <option value="1" <%=a3.Equals("1")?"selected='selected'":"" %>>网站</option>
                    <option value="2" <%=a3.Equals("2")?"selected='selected'":"" %>>电商</option>
                    <option value="3" <%=a3.Equals("3")?"selected='selected'":"" %>>移动端</option>
                    <option value="4" <%=a3.Equals("4")?"selected='selected'":"" %>>APP</option>
                </select>
        ...
        <table class="table" cellpadding="1" cellspacing="1">
            ...
            <tr>
                ...
                <td>
                    <a href="../ajax/artzd.ashx?id=<%=rv.id %>&fine=<%=rv.fine %>&url=../pro/?pid=<%=pid%>">
                ...
                <td><a href="update.aspx?id=<%=rv.id %>&pid=<%=pid %>" target="iframe">编辑</a> |
                     <a href="javaScript:void(0);" class="del" alt="../ajax/acticle.ashx?delid=<%=rv.id %>&cc=<%=pid %>&returnurl=../pro/">删除</a></td>
            </tr>
            <%} if (list.Count <= 0)
              {  %>
            <tr>
        ...
        var sort = function (sort) {
            if (sort.value != "") {
                if (sort.value != sort.getAttribute("val")) {
                    $.post("../ajax/artzd.ashx", { id: sort.getAttribute("id"), sort: sort.value, url: url }, function (data) {
